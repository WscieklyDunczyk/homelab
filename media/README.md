...
Polecenie wykonane w konsoli z komptera na którym mamy plik
...
```bash
    scp .env.example <użytkownik_kontenera_LXC>@<IP_KONTENERA>:/opt/docker-compose/pihole/
```

Później w kosnoli kontenera edytujemy zawartość pliku **.env**. Wpisujemy dane naszego VPN
...
- Radarr (`:7878`) → root foldery `/data/movies` i `/data/animacje`, download client Transmission
